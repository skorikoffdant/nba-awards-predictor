...
ALL_NBA_MODEL_PATH = MODELS_DIR / "all_nba_xgb_ranker.joblib"
ALL_ROOKIE_MODEL_PATH = MODELS_DIR / "all_rookie_xgb_ranker.joblib"
VALIDATION_REPORT_PATH = MODELS_DIR / "validation_report_xgb_ranker.json"
...
class RankerArtifact:
    def __init__(self, imputer: SimpleImputer, ranker: XGBRanker):
        self.imputer = imputer
        self.ranker = ranker

    def predict(self, X: pd.DataFrame) -> np.ndarray:
        X_imp = self.imputer.transform(X)
        return self.ranker.predict(X_imp)


def make_xgb_ranker(config: dict) -> XGBRanker:
    return XGBRanker(
...
def fit_ranker_model(
    ...
    train_df["_TARGET"] = target.values

    train_df = train_df.sort_values("SEASON_END_YEAR").reset_index(drop=True)

    X_train = train_df[feature_columns]
    y_train = train_df["_TARGET"].astype(float)

    group = (
        train_df.groupby("SEASON_END_YEAR", sort=False)
        .size()
        .to_numpy()
    )
    ...
    return RankerArtifact(imputer=imputer, ranker=ranker)
...
def build_all_nba_configs() -> list[dict]:
    ...
    for target_scheme, params in product(target_schemes, param_sets):
        cfg = dict(base)
        cfg.update(params)
...
def find_best_all_nba_model(
    ...
    print()
    print("=" * 80)
    print("ALL-NBA XGBRANKER SEARCH")
    print("=" * 80)
...
def find_best_all_rookie_model(
    ...
    print()
    print("=" * 80)
    print("ALL-ROOKIE XGBRANKER SEARCH")
    print("=" * 80)
    print(f"Configs to test: {len(configs)}")
    ...
    print()
    print("=" * 80)
    print("BEST ALL-ROOKIE XGBRANKER")
    print("=" * 80)

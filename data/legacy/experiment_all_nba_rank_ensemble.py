...
EXPERIMENT = "rank_ensemble"
DESCRIPTION = (
    "Rank ensemble. For each award, it combines the HGB classifier score with a "
    "vote-support regressor score by normalizing both rankings inside each season."
)
...
def load_dataset(spec, feature_set):
...
def evaluate_award(spec, args):
    df, votes_available = load_dataset(spec, args.feature_set)
    ...
    for season in range(args.backtest_start, args.backtest_end + 1):
        ...
        cls_model = fit_base_classifier(train_df, feature_cols, spec, args)
        reg_model = make_hgb_regressor(CONFIG["regressor_max_iter"], CONFIG["regressor_learning_rate"], CONFIG["regressor_max_leaf_nodes"], CONFIG["regressor_l2_regularization"])
        reg_model.fit(train_df[feature_cols], train_df["__VOTE_TARGET__"].astype(float))

        scored = add_classifier_scores(test_df, cls_model, feature_cols, spec, prefix="CLS")
        scored["REG_SCORE"] = reg_model.predict(scored[feature_cols])
        scored["CLS_NORM"] = safe_minmax(scored["CLS_EXPECTED_LABEL"])
        scored["REG_NORM"] = safe_minmax(scored["REG_SCORE"])
        scored["ENSEMBLE_SCORE"] = CONFIG["classifier_weight"] * scored["CLS_NORM"] + CONFIG["regressor_weight"] * scored["REG_NORM"]

        selected = select_top_candidates(scored, spec, score_col="ENSEMBLE_SCORE")
        prediction = assign_by_rank(selected, spec, order_col="CLS_EXPECTED_LABEL")
        ...
        if prediction_rows is not None:
            prediction_rows.extend(prediction_to_rows(prediction, season, args.experiment_name, spec.name, ["CLS_EXPECTED_LABEL", "REG_SCORE", "ENSEMBLE_SCORE"]))
